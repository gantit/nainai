  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (!env.RESEND_API_KEY) {
        return new Response(JSON.stringify({ mock: true, sent: false, reason: 'NO_API_KEY', preview: { to, subject, text } }), { status: 200, headers: { ...corsHeaders(), 'Content-Type': 'application/json' } });
      }
      const payload = {
      ...
      if (!resendResp.ok) {
        ...
        return new Response('Resend error: ' + errTxt, { status: 502, headers: corsHeaders() });
      }
